...
class ExecutorFixture {
	...
	/** The path, relative to the project-root, of a directory of image files that can be used as input. */
    private static final Path INPUT_DIRECTORY = Paths.get("src/test/resources/input/differentSizes");
    
    /** 
     * The path, relative to the project-root, of the configuration files that are bundles with an Anchor distribution
     *  and which describe the different possible tasks that can be run with Anchor.
     */
    private static final Path DEFAULT_EXPERIMENT = Paths.get("src/main/resources/config/defaultExperiment.xml");
    
	/**
     * Launch Anchor via a simulated CLI environment, run a task, and check that certain assertions are met.
     * 
     ...
     * @param otherArgs any other arguments that are passed to the CLI (in addition to existing input, output and task arguments)
     * @param tempDirectory a temporary directory, in which, we create an arbitrary path to an output-directory.
     * @param expectedFiles filenames that are expected to be found in the output-directory.
     */
    public static void runAndVerify(String taskName, List<String> otherArgs, Path tempDirectory, List<String> expectedFiles) {
    ...
    private static List<String> buildCommand(String taskName, List<String> otherArgs, Path outputDirectory) {
