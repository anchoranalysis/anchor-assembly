...
class DescribeTaskNames {
	 ...
	 */
	 public static void printTaskNames(PrintStream printTo, Multimap<String, String> tasks) {
        ...
        // Remove from the root, any tasks that have a subdirectory with the same name, as we will show them in describeSubdirectory instead
    ...
    /** Derive a new {@link SortedSet} containing only tasks that do not have a subdirectory with the same name. */
     ...
     * @return a string with newlines, describing all the predefined-tasks in this
     ...
     * @param filenames the filenames that exist for predefefined-tasks in {@code directory}.
     * @param taskInRoot true when a task exists in the root, with the same name as {@code directory}.
